...
func main() {
	...
	for fuelTarget := 0; oreRequired(fuelTarget, sorted, amounts, graph) < want; fuelTarget++ {
		prev = fuelTarget
	}
...
func oreRequired(fuelTarget int, sorted []node, amounts quantities, g graph) int {
	required := quantities{fuel: fuelTarget}
	for _, c := range sorted {
		if c == ore {
			break
		}
		f := required[c] / amounts[c]
		if required[c]%amounts[c] != 0 {
			f++
		}
		for _, e := range g[c] {
			required[e.to] += f * e.weight
		}
	}
